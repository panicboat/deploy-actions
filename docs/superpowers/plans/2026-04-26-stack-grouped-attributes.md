...
**Architecture:** `DeploymentTarget` を core 5 属性 + `attributes` Hash の構造へ変更し、コンストラクタで invariant を強制（`valid?` 廃止）。`WorkflowConfig` に `stack_attributes_for` / `required_attributes_for` を追加し、必須キーは `stack_conventions[].stacks[].required_attributes` の宣言で表現。format バリデーションは廃止。
...
`allow(config).to receive(:stack_convention_root).and_return('{service}')` の直後に：
...
(d) docker 用のスタブも必要。line 327 の `stack_conventions_for ... docker` mock の直後に：
